...
from forward_eval import realised_pnl_per_share, COMMISSION_PER_SHARE


# ── realised_pnl_per_share ───────────────────────────────────────────────────
...
def test_put_expires_worthless_returns_mid_minus_commission():
    ...
    expected = mid - COMMISSION_PER_SHARE
    assert realised_pnl_per_share(row, expiry_close=100.0) == pytest.approx(expected)
...
def test_put_expires_at_strike_returns_mid_minus_commission():
    ...
    row = _put_row(bid=1.50, ask=1.60)
    mid = (1.50 + 1.60) / 2.0
    pnl = realised_pnl_per_share(row, expiry_close=95.0)  # close == strike
    assert pnl == pytest.approx(mid - COMMISSION_PER_SHARE)
...
def test_put_partial_loss_capped_at_max_loss():
    ...
    pnl = realised_pnl_per_share(row, expiry_close=60.0)
    expected = -3.50 - COMMISSION_PER_SHARE
...
def test_put_small_loss_not_capped():
    ...
    pnl = realised_pnl_per_share(row, expiry_close=93.0)
    expected = -(95.0 - 93.0 - mid) - COMMISSION_PER_SHARE
...
def test_call_expires_worthless_returns_mid_minus_commission():
    ...
    mid = (1.50 + 1.60) / 2.0
    pnl = realised_pnl_per_share(row, expiry_close=100.0)  # below strike 105
    assert pnl == pytest.approx(mid - COMMISSION_PER_SHARE)
...
def test_call_partial_loss_capped():
    ...
    pnl = realised_pnl_per_share(row, expiry_close=150.0)
    expected = -3.50 - COMMISSION_PER_SHARE
    assert pnl == pytest.approx(expected)
...
def test_missing_ask_falls_back_to_bid():
    ...
    pnl = realised_pnl_per_share(row, expiry_close=100.0)
    expected = 1.50 - COMMISSION_PER_SHARE
    assert pnl == pytest.approx(expected)
...
def test_unknown_option_type_returns_zero():
    ...
    assert realised_pnl_per_share(row, expiry_close=100.0) == 0.0
...
def test_commission_constant_positive():
    ...
    assert 0 < COMMISSION_PER_SHARE < 0.10
